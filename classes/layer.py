...
class Layer:
    ...
    def forward_pass(self, inputs):
        ...
        Forward pass step
        """
        outputs = np.array([neuron.forward_pass(inputs) for neuron in self.neurons]) # Numpy array such that we can perform matrix multiplication at a later stage. Applies the forward pass to all neuron in the layer.
    ...
    def backward_pass(self, derivative_loss,lr):
        ...
        """
        derivative_inputs = np.zeros(self.input_size) # Initialising an array of zeros which will hold the total gradient of the loss
